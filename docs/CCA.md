...
The fitlered data from the preprocessing together with the a template containing sine and cosine signals from one reference frequency and its harmonics is put into the CCA module. The CCA module is imported from the sklearn library [link](https://scikit-learn.org/stable/modules/generated/sklearn.cross_decomposition.CCA.html). This function returns the corresponding weighting vectors explained above. Afterwards we apply these weighting vectors on the template and the data. Finally we van calculate the correlaltion between the signals and the template, this value is sotred. The process is repeated for every reference signal. The reference with the heighest correlation value is picked as the winner. 

The dots indicate how we could upgrade the regular CCA to the extended CCA. By adding training data to the template matrix we could increase the acuaracy of the method. This data is first averged for each frequency while keeping the different channels seperated. The final template will have dimensions: number of frequencies x number of channels x number of samples. 
