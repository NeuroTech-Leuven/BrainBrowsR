...
for package in ["pandas", "seaborn", "sklearn", "explorepy", "argparse", "time", "asyncio", "numpy", "scipy", "matplotlib", "sys"]:
...
def thresholding(threshold, data):
    for i in range(np.shape(data)[0]):
        ...
        Certainty = dominant_frequency - second_dominant_frequency
...
def main():
    # run command line: python online_pipeline.py -n Explore_849D -c 3 -w 2 -s 250 -d 20
    ...
    cca = Classifier(freqs=[9, 11, 13, 15], n_chan=3, t_min = 0, t_max = 4, fs=SAMPLING_RATE)  # classifier class

    # initialize constants
    CHANNEL_MASK = "011001000"
    ...
    while time.time() - start_time < STREAM_DURATION:
        ...
        preprocessor.filter_band(low_freq=0.5, high_freq=35, type_of_filter="bandpass", order_of_filter=5)
